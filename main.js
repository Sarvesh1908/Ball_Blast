...
GameArea=
{
	...
	start: function()
	{
		...
		myCanon = new Canon(180,650,100,30, "#d626cd" );
		myScore=new Score(590,40);
		myButton= new Button(10,730,80,730,590,730);
		...
		if(event.x < myButton.x2 || 50 && event.x > myButton.x2 &&
		    event.y < myButton.y2 + 20 && event.y > myButton.y2 - 20)
		    resumeGame();

		if(event.x < myButton.x3 + 50 && event.x > myButton.x3 &&
...
function Canon(x,y,width,height,color)
{
	...
	this.update=function()
	{
		...
		ctx.fillStyle="#48db65";
		ctx.beginPath();
		ctx.moveTo(0,700);
		ctx.lineTo(GameArea.canvas.width,700);
		ctx.lineTo(GameArea.canvas.width,800);
		ctx.lineTo(0,800);
		ctx.fill();
...
function Rocks(x,y,speedX,speedY,radius,color)
{
	...
	this.newPos=function()
	{
		...
		if(this.x > 700) {this.speedX = -this.speedX;}
...
function updateGameArea()
{
	...
	{
		myBullets.push(new Bullets(myCanon.x + (myCanon.width/2)+10,myCanon.y,-20,10,"yellow"));
		myBullets.push(new Bullets(myCanon.x + (myCanon.width/2)-10,myCanon.y,-20,10,"yellow"));
	}
	...
	if(GameArea.frameNo == 1 || GameArea.frameNo % rate==1)
	{
		...
		var rad = 20 + Math.floor( Math.random() * 30 );
			if(x1==0)
				...
				speed=-3;
		myRocks.push(new Rocks(x1,300,speed,0,rad,"#959d96"));
